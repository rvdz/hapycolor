...
class Node:
    ...
    def compute_value(self):
        value = 0
        vertices = [(self.qap.frequencies[i], c)
...
class QAP:
    ...
    Introduction
        ...
        The goal of the Quadratic Assignment Problem (QAP) is to assign
        :math:`n` facilities to :math:`n` locations in such a way as
        to minimize the assignment cost. The assignment cost is the sum, over
        all pairs, of the flow between a pair of facilities multiplied by the
        distance between their assigned locations.

    Formal definition

        The quadratic assignment problem is as follows: Given two sets,
        :math:`P` ("facilities") and :math:`L` ("locations"), of equal size,
        together with a weight function:

        .. math::
            w : P \times P \rightarrow \mathbb{R}

        And a distance function:

        .. math::
            d : L \times L \rightarrow \mathbb{R}`

        Find the bijection or "assignment":
    ...
    How it is used in Hapycolor
        ...
        assigned to the syntactic groups that appear the most in a program's
        code.

    How to use it

        This class takes two arguments: the colors and the weights, e.g when
        exporting to vim, they represent the inverse of the frequencies of the
        different syntactic groups. Using the original formulation of the
        problem, the "flow" between "facility" `a` to "facility" `b` is the
        inverse of the product of both frequencies. Then the implemented
        algorithm solves the problem by exploring the different possible
        combination following a Branch and Bound pattern. Eventually, it will
        return a list of tuples `(color, frequency)`.
